...
const AVATAR_COLORS = [
  ...
  "#ef4444","#22c55e","#3b82f6","#ec4899","#0ea5e9",
];

const SELECT = `
  SELECT id, member_number, name, phone, email, id_passport,
         to_char(joined_date, 'YYYY-MM-DD') AS joined_date,
         status, avatar_color, photo_url, investment_amount,
         created_at, updated_at
  FROM investors
`;

async function nextMemberNumber(client: any): Promise<number> {
  const { rows } = await client.query("SELECT COALESCE(MAX(member_number),0)+1 AS next FROM investors");
  return rows[0].next;
}
...
// ─── GET /api/investors/check-phone/:phone ────────────────────────────────────
...
router.get("/:id", async (req: Request, res: Response) => {
  try {
    ...
    if (!rows[0]) return res.status(404).json({ success: false, error: "Investor not found" });
...
router.post("/", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (existing.rows.length > 0) { await client.query("ROLLBACK"); return res.status(409).json({ success: false, error: `Phone registered as a ${existing.rows[0].member_type}` }); }

    const countResult = await client.query("SELECT COUNT(*) FROM investors");
    ...
    const { rows } = await client.query(
      `INSERT INTO investors (member_number, name, phone, email, id_passport, joined_date, status, avatar_color, photo_url)
...
router.patch("/:id", async (req: Request, res: Response) => {
  ...
  try {
    await client.query("BEGIN");
    const current = await client.query("SELECT * FROM investors WHERE id=$1", [req.params.id]);
...
router.patch("/:id/status", async (req: Request, res: Response) => {
  try {
    ...
    const { rows } = await pool.query(
      "UPDATE investors SET status=$1 WHERE id=$2 RETURNING id,member_number,name,status,avatar_color,photo_url",
...
router.delete("/:id", async (req: Request, res: Response) => {
  try {
    const { rowCount } = await pool.query("DELETE FROM investors WHERE id=$1", [req.params.id]);
