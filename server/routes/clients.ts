...
const SELECT = `
  SELECT id, member_number, name, phone, email, id_passport,
         ...
         status, avatar_color, photo_url, loan_balance,
         created_at, updated_at
  FROM clients
`;

async function nextMemberNumber(client: any): Promise<number> {
  const { rows } = await client.query("SELECT COALESCE(MAX(member_number),0)+1 AS next FROM clients");
  return rows[0].next;
}
...
// ─── GET /api/clients/check-phone/:phone ──────────────────────────────────────
...
router.post("/", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { rows } = await client.query(
      `INSERT INTO clients (member_number, name, phone, email, id_passport, joined_date, status, avatar_color, photo_url)
...
router.patch("/:id", async (req: Request, res: Response) => {
  ...
  try {
    await client.query("BEGIN");
    const current = await client.query("SELECT * FROM clients WHERE id=$1", [req.params.id]);
...
router.patch("/:id/status", async (req: Request, res: Response) => {
  try {
    ...
    const { rows } = await pool.query(
      "UPDATE clients SET status=$1 WHERE id=$2 RETURNING id,member_number,name,status,avatar_color,photo_url",
